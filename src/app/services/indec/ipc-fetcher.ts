  ...
  function extractDateFromValue(value: any): string | null {
    ...
    if (typeof value === 'number') {
      try {
        ...
        if (value > 10000) { // Evitar confundir con otros números pequeños
        ...
        console.warn(`No se pudo convertir el valor numérico ${value} a fecha:`, e);
      }
      return null;
    }
  ...
  export async function fetchIPCData(): Promise<Omit<IpcRow, 'id'>[]> {
    try {
      ...
      try {
        ...
        console.warn(`Error descargando el archivo del mes actual. Intentando con el mes anterior...`);
        // Si falla, intentar con el mes anterior (común al inicio de un nuevo mes)
        const prevMonth = String(((now.getMonth() + 11) % 12) + 1).padStart(2, '0');
      ...
      const targetSheetName = workbook.SheetNames.find(
        name => name.includes("ndices IPC Cobertura Nacional") || name.includes("Indices IPC Cobertura Nacional")
      );
      
      if (!targetSheetName) {
        throw new Error('No se encontró la hoja "Índices IPC Cobertura Nacional" en el Excel');
      }
      ...
      const regions: Array<{name: string, shortName: string, startRow: number}> = [];
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i] && data[i][0]) {
          ...
          const regionIdx = regionNames.findIndex(name => 
            cellValue === name || 
          ...
          if (regionIdx !== -1) {
            ...
            if (data[i][1] !== null && data[i][1] !== undefined) {
              ...
              console.log(`  - Valor en columna B: ${data[i][1]}, tipo: ${typeof data[i][1]}`);
              if (data[i][2] !== null && data[i][2] !== undefined) {
                console.log(`  - Valor en columna C: ${data[i][2]}, tipo: ${typeof data[i][2]}`);
              }
            }
          }
        }
      ...
      regions.forEach(region => {
        console.log(`- ${region.name} (${region.shortName}): fila ${region.startRow+1}`);
      });
      
      if (regions.length === 0) {
        // Si no encontramos regiones, vamos a imprimir algunos valores para depuración
        console.log("No se encontraron regiones. Mostrando primeras filas para depuración:");
        for (let i = 0; i < Math.min(20, data.length); i++) {
          console.log(`Fila ${i+1}:`, data[i] ? data[i].slice(0, 3) : null);
        }
        throw new Error('No se pudieron identificar regiones en el Excel del IPC');
      }
      ...
      console.log("Intentando extraer fechas de la fila:", regions[0].startRow + 1);
      ...
      for (let i = 1; i < headerRow.length; i++) {
        if (headerRow[i] !== null && headerRow[i] !== undefined) {
          console.log(`Valor en columna ${i+1} (índice ${i}):`, headerRow[i], "tipo:", typeof headerRow[i]);
      ...
      if (dates.length === 0 && regions[0].startRow + 1 < data.length) {
        console.log("No se encontraron fechas en la fila de encabezado. Intentando con la siguiente fila.");
      ...
      if (dates.length === 0) {
        console.log("No se encontraron fechas en las primeras filas. Intentando generar fechas automáticamente.");
        
        // Contar columnas con datos y generar fechas
        const columnCount = headerRow.filter(x => x !== null && x !== undefined).length - 1; // -1 por la columna A
        ...
        if (columnCount > 0) {
          ...
          for (let i = 0; i < columnCount; i++) {
            ...
            // Crear fecha ISO
            const date = `${targetYear}-${String(targetMonth + 1).padStart(2, '0')}-01`;
      ...
      for (const region of regions) {
        ...
        for (let i = region.startRow + 1; i < region.startRow + 10 && i < data.length; i++) {
        ...
        if (!foundNivelGeneral) {
          console.warn(`No se encontró "Nivel general" para la región ${region.name}`);
        ...
        for (let j = 0; j < dates.length; j++) {
          ...
          if (colIndex < data[nivelGeneralRow].length && 
              data[nivelGeneralRow][colIndex] !== null && 
              data[nivelGeneralRow][colIndex] !== undefined) {
            
            let value = typeof data[nivelGeneralRow][colIndex] === 'number' 
                      ? data[nivelGeneralRow][colIndex] 
                      : typeof data[nivelGeneralRow][colIndex] === 'string' && !isNaN(parseFloat(data[nivelGeneralRow][colIndex])) 
        ...
        for (let i = 0; i < 15 && currentRow + i < data.length; i++) {
          ...
          const matchedRubroIndex = expectedRubros.findIndex(rubro => 
            cellValue === rubro || cellValue.includes(rubro) || rubro.includes(cellValue)
          );
          ...
          if (matchedRubroIndex !== -1) {
            ...
            for (let j = 0; j < dates.length; j++) {
              ...
              if (colIndex < data[rubroRow].length && 
                  data[rubroRow][colIndex] !== null && 
                  data[rubroRow][colIndex] !== undefined) {
                
                let value = typeof data[rubroRow][colIndex] === 'number' 
                          ? data[rubroRow][colIndex] 
                          : typeof data[rubroRow][colIndex] === 'string' && !isNaN(parseFloat(data[rubroRow][colIndex])) 
        ...
        console.log(`Procesados ${rubroCount} rubros para la región ${region.name}`);
        
        // Buscar la sección de "Categorías"
        let categoriasHeaderRow = -1;
        
        // Buscar hasta 15 filas después del último rubro
        for (let i = nivelGeneralRow + rubroCount + 1; i < nivelGeneralRow + rubroCount + 15 && i < data.length; i++) {
          if (data[i] && data[i][0] === "Categorías") {
        ...
        if (categoriasHeaderRow !== -1) {
          ...
          // Verificar hasta 5 filas después del encabezado de categorías
          for (let i = 0; i < 5 && categoriasHeaderRow + 1 + i < data.length; i++) {
            ...
            const matchedCategoriaIndex = expectedCategorias.findIndex(cat => 
              cellValue === cat || cellValue.includes(cat) || cat.includes(cellValue)
            );
            ...
            if (matchedCategoriaIndex !== -1) {
              ...
              for (let j = 0; j < dates.length; j++) {
                ...
                if (colIndex < data[categoriaRow].length && 
                    data[categoriaRow][colIndex] !== null && 
                    data[categoriaRow][colIndex] !== undefined) {
                  
                  let value = typeof data[categoriaRow][colIndex] === 'number' 
                            ? data[categoriaRow][colIndex] 
                            : typeof data[categoriaRow][colIndex] === 'string' && !isNaN(parseFloat(data[categoriaRow][colIndex])) 
                  ...
                  if (value !== null) {
                    processedData.push({
                      date: dates[j],
                      component: categoria,
                      component_code: `CAT_${categoria.toUpperCase()}`,
          ...
          console.log(`Procesadas ${categoriaCount} categorías para la región ${region.name}`);
        }
        ...
        const startSearchRow = categoriasHeaderRow !== -1 ? categoriasHeaderRow + 5 : nivelGeneralRow + rubroCount + 5;
        
        for (let i = startSearchRow; i < startSearchRow + 10 && i < data.length; i++) {
          if (data[i] && (data[i][0] === "Bienes y servicios" || 
              (typeof data[i][0] === 'string' && data[i][0].includes("Bienes y servicios")))) {
        ...
        if (bienesYServiciosHeaderRow !== -1) {
          ...
          for (let i = 0; i < 3 && bienesYServiciosHeaderRow + 1 + i < data.length; i++) {
            ...
            const matchedBYSIndex = expectedBienesPlusServicios.findIndex(item => 
              cellValue === item || cellValue.includes(item) || item.includes(cellValue)
            );
            ...
            if (matchedBYSIndex !== -1) {
              ...
              for (let j = 0; j < dates.length; j++) {
                ...
                if (colIndex < data[bysRow].length && 
                    data[bysRow][colIndex] !== null && 
                    data[bysRow][colIndex] !== undefined) {
                  
                  let value = typeof data[bysRow][colIndex] === 'number' 
                            ? data[bysRow][colIndex] 
                            : typeof data[bysRow][colIndex] === 'string' && !isNaN(parseFloat(data[bysRow][colIndex])) 
          ...
          console.log(`Procesados ${bysCount} componentes de bienes y servicios para la región ${region.name}`);
        }
      }
      
      console.log(`Datos IPC procesados: ${processedData.length} registros totales`);
      console.log(`Regiones procesadas: ${[...new Set(processedData.map(item => item.region))].join(', ')}`);
      ...
      throw new Error(`Error al obtener datos del IPC: ${(error as Error).message}`);
    }
  }
