...
export default function ModernEMAEPage({ initialData }: ModernEMAEPageProps) {
  const [emaeData, setEmaeData] = useState<any>(initialData?.latest || null);
  const [loading, setLoading] = useState(!initialData);
  const [error, setError] = useState<string | null>(null);

  // Obtener datos actuales del EMAE solo si no hay datos iniciales
  useEffect(() => {
    const fetchLatestEMAE = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/emae/latest?_t=${timestamp}`, {
          cache: 'no-store',
          headers: {
